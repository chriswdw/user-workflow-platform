...
class KafkaDomainEventPublisherTest {
  ...
  void publish_withCorrelationId_sendsRecordWithBothHeaders() {
    DomainEvent event =
        new DomainEvent(
            "evt-1", "tenant-1", "wi-1", "corr-1", "WORK_ITEM_CREATED", Instant.now(), Map.of());
  ...
  void publish_withoutCorrelationId_omitsCorrelationHeader() {
    DomainEvent event =
        new DomainEvent(
            "evt-2", "tenant-1", "wi-2", null, "WORK_ITEM_UPDATED", Instant.now(), Map.of());
  ...
  void publish_serialisationFailure_logsAndDoesNotSend() {
    DomainEvent event =
        new DomainEvent(
            "evt-3", "tenant-1", "wi-3", "corr-3", "WORK_ITEM_FAILED", Instant.now(), Map.of());
