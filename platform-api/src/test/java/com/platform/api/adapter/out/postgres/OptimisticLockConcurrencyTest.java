...
/**
 * Verifies the optimistic lock guard under real concurrent PostgreSQL writes. The sequential
 * version-mismatch scenario is already covered in WorkItemJdbcRepositoryTest. This test uses two
 * threads racing at the same instant — the only way to prove the WHERE version = :version clause
 * works correctly under actual concurrent load.
 *
 * <p>Repeated 5 times to rule out false-passes from lucky scheduling.
 */
class OptimisticLockConcurrencyTest {
  ...
  private final WorkItemJdbcRepository repository = new WorkItemJdbcRepository(jdbc, objectMapper);
  ...
  void concurrentSaves_exactlyOneSucceedsAndOneThrowsOptimisticLockFailure()
    ...
    startGun.countDown();
    assertThat(doneLatch.await(10, TimeUnit.SECONDS)).as("threads did not finish in time").isTrue();

    assertThat(unexpectedErrors).as("no unexpected exceptions").isEmpty();
    assertThat(successes.get()).as("exactly one save succeeds").isEqualTo(1);
    assertThat(failures.get()).as("exactly one save is rejected").isEqualTo(1);
