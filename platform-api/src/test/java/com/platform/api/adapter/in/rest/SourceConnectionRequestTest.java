 ...
 * the correct ConnectionConfig sealed variant. Only the KAFKA branch was exercised by the
 * Cucumber source-connection scenarios; DB_POLL and FILE_SHARE (and the pollIntervalSeconds
...
class SourceConnectionRequestTest {
    ...
    void toConnectionConfigBuildsKafkaConfig() {
        var request = new SourceConnectionRequest("conn", "Conn", "KAFKA",
                Map.of("bootstrapServers", "localhost:9092", "topicName", "work-items"), null);
    ...
    void toConnectionConfigBuildsDbPollConfigWithExplicitPollInterval() {
        var request = new SourceConnectionRequest("conn", "Conn", "DB_POLL",
                Map.of("jdbcUrl", "jdbc:postgresql://localhost/db",
                        "query", "SELECT 1",
    ...
    void toConnectionConfigBuildsFileShareConfig() {
        var request = new SourceConnectionRequest("conn", "Conn", "FILE_SHARE",
                Map.of("path", "/mnt/inbound", "filePattern", "*.csv"), null);
