...
class GlobalExceptionHandlerTest {
  ...
  void handleMisconfiguration_hidesInternalDetailAndLogsCorrelationId() {
    ...
    try {
      ProblemDetail pd =
          handler.handleMisconfiguration(new WorkflowConfigNotFoundException("TRADE", "tenant-1"));
  ...
  void handleUnprocessable_coversAllRegisteredUnprocessableTypes() {
    assertThat(
            handler
                .handleUnprocessable(new IncompleteSubmissionException("missing field"))
                .getStatus())
        .isEqualTo(HttpStatus.UNPROCESSABLE_CONTENT.value());
    assertThat(
            handler
                .handleUnprocessable(new InvalidTransitionException("bad transition"))
                .getStatus())
        .isEqualTo(HttpStatus.UNPROCESSABLE_CONTENT.value());
