...
/**
 * Verifies that all three JDBC repositories enforce tenant boundaries correctly. Each test inserts
 * data for tenant-A and asserts that tenant-B sees nothing, even when IDs overlap. This is a
 * regulatory requirement for a multi-tenant financial services platform — tenant data leakage is a
 * serious incident.
 */
class MultiTenantIsolationTest {
  ...
  private final WorkItemJdbcRepository workItems = new WorkItemJdbcRepository(jdbc, objectMapper);
  ...
  void findByTenantAndWorkflowType_doesNotReturnWorkItemsOwnedByDifferentTenant() {
    ...
    List<WorkItem> results =
        workItems.findByTenantAndWorkflowType("tenant-A", "SETTLEMENT_EXCEPTION");

    assertThat(results).hasSize(2).extracting(WorkItem::tenantId).containsOnly("tenant-A");
  }
  ...
  void findByTenantAndWorkflowTypeAndType_doesNotReturnDocumentsOwnedByDifferentTenant() {
    ...
    assertThat(
            configDocs.findByTenantAndWorkflowTypeAndType(
  ...
  private static void insertWorkItemWithIdemKey(
      String id, String tenantId, String workflowType, String idemKey) {
