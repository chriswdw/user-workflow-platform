...
public class WorkflowTypeSubmissionJdbcRepository implements IWorkflowTypeSubmissionRepository {
  ...
  public WorkflowTypeSubmissionJdbcRepository(
  ...
  public Optional<WorkflowTypeSubmission> findById(String tenantId, String submissionId) {
    String sql =
        """
                ...
                """;
    var params =
        new MapSqlParameterSource().addValue(COL_TENANT_ID, tenantId).addValue("id", submissionId);
    return jdbc.query(sql, params, this::mapRow).stream().findFirst();
  }

  @Override
  public List<WorkflowTypeSubmission> findByTenantAndStatus(
      String tenantId, SubmissionStatus status) {
    String sql =
        """
                SELECT s.*, ss.display_name AS status_display_name
                FROM workflow_type_submissions s
  ...
  @Override
  public List<WorkflowTypeSubmission> findByTenantAndStatusAndUser(
      String tenantId, SubmissionStatus status, String userId) {
    String sql =
        """
                SELECT s.*, ss.display_name AS status_display_name
                FROM workflow_type_submissions s
  ...
  public void deleteById(String tenantId, String submissionId) {
    jdbc.update(
        "DELETE FROM workflow_type_submissions WHERE tenant_id = :tenantId AND id = :id",
        new MapSqlParameterSource().addValue(COL_TENANT_ID, tenantId).addValue("id", submissionId));
  }
