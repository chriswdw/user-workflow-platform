  ...
  public void publish(DomainEvent event) {
    try {
      ...
      ProducerRecord<String, String> producerRecord =
          new ProducerRecord<>(topic, event.workItemId(), payload);
      if (event.correlationId() != null) {
        producerRecord
            .headers()
            .add(
                new RecordHeader(
                    "X-Correlation-ID", event.correlationId().getBytes(StandardCharsets.UTF_8)));
      }
      producerRecord
          .headers()
      ...
      log.error(
          "correlationId={} Failed to serialise domain event type={}",
          event.correlationId(),
          event.eventType(),
          e);
    }
