...
public class WorkItemJdbcRepository
  ...
  public Optional<WorkItem> findById(String tenantId, String workItemId) {
    String sql =
        """
                ...
                """;
    var params =
        new MapSqlParameterSource().addValue("id", workItemId).addValue(TENANT_ID, tenantId);
  ...
  public WorkItem save(WorkItem w) {
    ...
    return new WorkItem(
        ...
        w.priorityScore(),
        w.priorityLevel(),
        w.priorityLastCalculatedAt(),
        w.pendingCheckerId(),
        w.pendingCheckerTransition(),
        w.version() + 1,
        w.makerUserId(),
        w.createdAt(),
        now);
  ...
  private WorkItem mapRow(ResultSet rs, int rowNum) throws SQLException {
    Integer priorityScore =
        rs.getObject("priority_score") != null ? rs.getInt("priority_score") : null;
    return new WorkItem(
        rs.getString("id"),
