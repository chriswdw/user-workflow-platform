...
public class SourceConnectionJdbcRepository implements ISourceConnectionRepository {
  ...
  public SourceConnectionJdbcRepository(
  ...
  public List<SourceConnection> findAll() {
    return jdbc.query(
        "SELECT * FROM source_connections ORDER BY display_name", Map.of(), this::mapRow);
  }

  @Override
  public List<SourceConnection> findAccessibleByTenantAndType(
      String tenantId, ConnectionType type) {
    String sql =
        """
                SELECT sc.*
  ...
  public void delete(String id) {
    jdbc.update(
        "DELETE FROM source_connections WHERE id = :id", new MapSqlParameterSource("id", id));
  }
  ...
  private String configToJson(ConnectionConfig config) {
    try {
      Map<String, Object> map =
          switch (config) {
            case ConnectionConfig.KafkaConfig(var bootstrapServers, var topicName) ->
                Map.of(
                    "type", "KAFKA",
                    "bootstrapServers", bootstrapServers,
                    "topicName", topicName);
            case ConnectionConfig.DbPollConfig(var jdbcUrl, var query, var pollIntervalSeconds) ->
                Map.of(
                    "type", "DB_POLL",
                    "jdbcUrl", jdbcUrl,
                    "query", query,
                    "pollIntervalSeconds", pollIntervalSeconds);
            case ConnectionConfig.FileShareConfig(var path, var filePattern) ->
                Map.of(
                    "type", "FILE_SHARE",
                    "path", path,
  ...
  private ConnectionConfig parseConfig(String json, ConnectionType type) {
    try {
      ...
      return switch (type) {
        case KAFKA ->
            new ConnectionConfig.KafkaConfig(
                (String) map.get("bootstrapServers"), (String) map.get("topicName"));
        case DB_POLL ->
        ...
        case FILE_SHARE ->
            new ConnectionConfig.FileShareConfig(
                (String) map.get("path"), (String) map.get("filePattern"));
      };
    } catch (JacksonException e) {
