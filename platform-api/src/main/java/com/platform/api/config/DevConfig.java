...
/**
 * In-memory fallback implementations for all port interfaces. Each bean is conditional on no other
 * implementation being present — when a real adapter (e.g. JPA + PostgreSQL) is registered, these
 * step aside automatically. No profile flag required.
 */
@Configuration
@Profile("!local")
public class DevConfig {
  ...
  @Bean
  @ConditionalOnMissingBean
  public ITransitionWorkItemUseCase transitionWorkItemUseCase(
      ConcurrentMap<String, WorkItem> store) {
    return (TransitionCommand cmd) -> {
      String key = storeKey(cmd.tenantId(), cmd.workItemId());
      WorkItem current = store.get(key);
      if (current == null)
        throw new IllegalStateException("Work item not found: " + cmd.workItemId());
  ...
  @Bean
  @ConditionalOnMissingBean
  public IQueryAuditTrailUseCase queryAuditTrailUseCase(
      ConcurrentMap<String, List<AuditEntry>> store) {
  ...
  public ILoadConfigUseCase loadConfigUseCase() {
    return (tenantId, workflowType, configType) -> {
      if (ConfigType.DETAIL_VIEW_CONFIG.equals(configType) && SETTLEMENT_EX.equals(workflowType)) {
        return new ConfigDocument(
            "dev-dvc-settlement",
            tenantId,
            workflowType,
            ConfigType.DETAIL_VIEW_CONFIG,
            seedDetailViewConfig(),
            "1",
            true);
      }
      throw new ConfigNotFoundException("No dev config for " + configType + "/" + workflowType);
    };
  }
    ...
    return Map.of(
        ...
        List.of(
            Map.of(
                ...
                "PRIMARY",
                K_VISIBLE_STATES,
                List.of(UNDER_REVIEW, ESCALATED),
                K_VISIBLE_ROLES,
                List.of(ANALYST, SUPERVISOR),
                "confirmationRequired",
                true,
                "confirmationMessage",
                "Close this exception as resolved?",
                "inputFields",
                List.of(
                    Map.of(
                        K_FIELD,
            ...
            Map.of(
                F_TRADE,
                    Map.of(
                        ...
                        "2024-10-17",
                        F_NOTIONAL_AMOUNT,
                        Map.of(F_AMOUNT, "15000000.00", F_CURRENCY, "GBP")),
                F_COUNTERPARTY,
                    Map.of(
            ...
            Map.of(
                F_TRADE,
                    Map.of(
                        ...
                        "2024-10-18",
                        F_NOTIONAL_AMOUNT,
                        Map.of(F_AMOUNT, "2500000.00", F_CURRENCY, "EUR")),
                F_COUNTERPARTY,
                    Map.of(
            ...
            Map.of(
                F_TRADE,
                    Map.of(
                        ...
                        "2024-10-15",
                        F_NOTIONAL_AMOUNT,
                        Map.of(F_AMOUNT, "50000000.00", F_CURRENCY, "USD")),
                F_COUNTERPARTY,
                    Map.of(
            ...
            Map.of(
                F_TRADE,
                    Map.of(
                        ...
                        "2024-10-14",
                        F_NOTIONAL_AMOUNT,
                        Map.of(F_AMOUNT, "175000.00", F_CURRENCY, "GBP")),
                F_COUNTERPARTY,
                    Map.of(
                        "name", "HSBC Holdings",
            ...
            Map.of(
                F_TRADE,
                    Map.of(
                        ...
                        "2024-10-20",
                        F_NOTIONAL_AMOUNT,
                        Map.of(F_AMOUNT, "8750000.00", F_CURRENCY, "CHF")),
                F_COUNTERPARTY,
                    Map.of(
                        "name", "UBS Group AG",
  ...
  private static Map<String, List<AuditEntry>> seedAudit() {
    ...
    return Map.of(
        storeKey(TENANT_1, WI_001),
            List.of(
                auditEntry(TENANT_1, WI_001, AuditEventType.INGESTION, null, UNDER_REVIEW, base),
                auditEntry(
                    TENANT_1,
                    WI_001,
