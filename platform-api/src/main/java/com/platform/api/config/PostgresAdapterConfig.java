...
/**
 * Activated only when spring.datasource.url is configured. Each bean displaces the
 * corresponding @ConditionalOnMissingBean fallback in DevConfig.
 */
@Configuration
...
public class PostgresAdapterConfig {
  ...
  @Bean
  public WorkItemJdbcRepository workItemJdbcRepository(
      NamedParameterJdbcTemplate jdbc, ObjectMapper objectMapper) {
    return new WorkItemJdbcRepository(jdbc, objectMapper);
  }

  @Bean
  public AuditEntryJdbcRepository auditEntryJdbcRepository(
      NamedParameterJdbcTemplate jdbc, ObjectMapper objectMapper) {
    return new AuditEntryJdbcRepository(jdbc, objectMapper);
  }

  @Bean
  public ConfigDocumentJdbcRepository configDocumentJdbcRepository(
      NamedParameterJdbcTemplate jdbc, ObjectMapper objectMapper) {
    return new ConfigDocumentJdbcRepository(jdbc, objectMapper);
  }

  @Bean
  public WorkflowConfigJdbcRepository workflowConfigJdbcRepository(
      NamedParameterJdbcTemplate jdbc, ObjectMapper objectMapper) {
  ...
  public ConfigService configService(
      ConfigDocumentJdbcRepository configRepo, MeterRegistry meterRegistry) {
    return new ConfigService(configRepo, meterRegistry);
  }

  @Bean
  public WorkflowService workflowService(
      WorkItemJdbcRepository workItemRepo,
      WorkflowConfigJdbcRepository workflowConfigRepo,
      AuditEntryJdbcRepository auditRepo,
      IDomainEventPublisher eventPublisher,
      MeterRegistry meterRegistry) {
    return new WorkflowService(
        workItemRepo, workflowConfigRepo, auditRepo, eventPublisher, meterRegistry);
  }
  ...
  public SubmissionDraftService submissionDraftService(
      WorkflowTypeSubmissionJdbcRepository repo, AuditEntryJdbcRepository auditRepo) {
  ...
  @Bean
  public SubmissionQueryService submissionQueryService(WorkflowTypeSubmissionJdbcRepository repo) {
