...
/**
 * WorkItem is the central domain entity; every state change goes through one of its "with"
 * methods, which must return a new immutable instance with exactly one field changed (plus
 * updatedAt advancing) and every other field carried over unchanged. This invariant underpins the
...
class WorkItemTest {
    ...
    private static WorkItem baseWorkItem() {
        ...
        return new WorkItem(
                "wi-1", "tenant-1", "TRADE_BREAK", "corr-1", "cfg-v1",
                SourceType.KAFKA, "topic-ref", "idem-1",
                "OPEN", "group-A", true,
                Map.of("trade", Map.of("ref", "TRD-001")),
                50, "MEDIUM", created,
                null, null,
                1, "maker-1", created, created);
    }
