...
/**
 * SourceConnectionAccess records which tenant was granted access to a shared source connection,
 * and by whom — the audit trail for cross-tenant connection sharing. Plain data carrier, no
 * validation logic, but previously had no direct test.
 */
class SourceConnectionAccessTest {
    ...
    void carriesGrantDetails() {
        ...
        var access = new SourceConnectionAccess(
                "access-1", "conn-1", "tenant-2", "admin-1", grantedAt);
