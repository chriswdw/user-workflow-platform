...
final class SubmissionGuards {
  ...
  static void assertStatus(WorkflowTypeSubmission s, SubmissionStatus expected, String op) {
    if (s.status() != expected) {
      throw new IllegalStateException(
          "Cannot " + op + " a submission in status " + s.status() + "; expected " + expected);
    }
  }
  ...
  static void publishConfigDocuments(
      WorkflowTypeSubmission submission, IConfigDocumentWriter writer) {
    ...
    writer.saveAll(
        List.of(
            toDoc(
                tenantId,
                workflowType,
                ConfigType.WORKFLOW_TYPE_DEFINITION,
                dc.workflowTypeDefinition(),
                version),
            toDoc(
                tenantId,
                workflowType,
                ConfigType.FIELD_TYPE_REGISTRY,
                dc.fieldTypeRegistry(),
                version),
            toDoc(
                tenantId,
                workflowType,
                ConfigType.INGESTION_SOURCE_CONFIG,
                dc.ingestionSourceConfig(),
                version),
            toDoc(tenantId, workflowType, ConfigType.WORKFLOW_CONFIG, dc.workflowConfig(), version),
            toDoc(tenantId, workflowType, ConfigType.BLOTTER_CONFIG, dc.blotterConfig(), version),
            toDoc(
                tenantId,
                workflowType,
  ...
  static AuditEntry submissionAuditEntry(
  ...
  private static ConfigDocument toDoc(
      String tenantId,
      String workflowType,
    ...
    return new ConfigDocument(
        UUID.randomUUID().toString(), tenantId, workflowType, configType, content, version, true);
  }
}
