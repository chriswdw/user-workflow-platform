...
/**
 * Domain service implementing config loading and cross-schema validation. No framework dependencies
 * — constructor-injected output port only.
 *
 * <p>Loading invariant: exactly one active document must exist per (tenantId, workflowType,
 * configType). Zero → ConfigNotFoundException. More than one → ConfigIntegrityException.
 *
 * <p>Validation checks: 1. No duplicate active configs per (configType, workflowType) 2.
 * routing-config.defaultGroup references an active resolution-group
 */
public class ConfigService implements ILoadConfigUseCase, IValidateConfigsUseCase {
  ...
  public ConfigDocument loadActive(String tenantId, String workflowType, ConfigType configType) {
    List<ConfigDocument> active =
        repository.findByTenantAndWorkflowTypeAndType(tenantId, workflowType, configType).stream()
  ...
  private static void checkDuplicateActiveConfigs(
      List<ConfigDocument> active, List<ConfigValidationViolation> violations) {
    Map<String, Long> counts =
        active.stream()
            .collect(
                Collectors.groupingBy(
                    d -> d.configType() + ":" + d.workflowType(), Collectors.counting()));
      ...
      if (counts.get(key) > 1) {
        violations.add(
            new ConfigValidationViolation(
                "duplicate active " + doc.configType() + " for workflowType=" + doc.workflowType(),
                doc.configType(),
                doc.tenantId(),
                doc.workflowType()));
   ...
   */
  private static void checkRoutingConfigDefaultGroups(
      List<ConfigDocument> active, List<ConfigValidationViolation> violations, String tenantId) {
