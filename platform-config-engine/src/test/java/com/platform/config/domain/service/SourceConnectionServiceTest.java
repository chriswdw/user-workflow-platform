...
class SourceConnectionServiceTest {
    ...
    private static SourceConnection draftConnection(String name) {
        ...
        return new SourceConnection(
                "ignored-id", name, "Display " + name, ConnectionType.KAFKA,
                new ConnectionConfig.KafkaConfig("localhost:9092", "topic-" + name),
                "vault://creds", "admin-1", now, now);
    }
    ...
    void updateMergesProvidedFieldsOverExisting() {
        ...
        SourceConnection partialUpdate = new SourceConnection(
    ...
    void updateAdvancesUpdatedAtButPreservesCreatedAt() {
        ...
        SourceConnection updated = service.update(new SourceConnection(
    ...
    void updateThrowsWhenConnectionDoesNotExist() {
        SourceConnection nonExistent = new SourceConnection(
                "missing-id", "x", null, null, null, null, null, null, null);

        assertThat(catchThrowableMessage(() -> service.update(nonExistent)))
                .contains("missing-id");
    }
