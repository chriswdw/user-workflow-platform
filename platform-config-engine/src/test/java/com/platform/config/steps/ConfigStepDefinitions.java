...
/**
 * Cucumber step definitions for the config engine feature. Cucumber creates a new instance per
 * scenario — all fields are per-scenario state. Domain service wired with in-memory doubles; no
...
public class ConfigStepDefinitions {
  ...
  @Given(
      "tenant {string} has an active routing config for workflow type {string} with defaultGroup {string}")
  public void tenantHasActiveRoutingConfigWithDefaultGroup(
      String tenantId, String workflowType, String defaultGroup) {
    repo.save(
        routingConfig("config-r-" + (++docCounter), tenantId, workflowType, defaultGroup, true));
  }
  ...
  @When(
      "I attempt to load the active ROUTING_CONFIG for tenant {string} and workflow type {string}")
  ...
  private static ConfigDocument routingConfig(
      String id, String tenantId, String workflowType, String defaultGroup, boolean active) {
