...
/**
 * Domain service implementing the routing use case. No framework dependencies —
 * constructor-injected output ports only.
 *
 * <p>Algorithm: 1. Load active RoutingConfig for (tenantId, workflowType) 2. Evaluate active rules
 * in ascending priority order; first match wins 3. If no rule matches, assign defaultGroup and set
 * routedByDefault=true 4. Write AuditEntry (ASSIGNMENT or ROUTING_FALLBACK)
 *
 * <p>DOMAIN INVARIANT: assignedGroupId is never null. An unrouted work item is not permitted.
 */
public class RoutingService implements IRouteWorkItemUseCase {
  ...
  public RoutingService(
      IRoutingConfigRepository routingConfigRepository,
  ...
  public RoutingResult route(WorkItemToRoute workItem) {
    ...
    RoutingConfig config =
        routingConfigRepository
            ...
            .orElseThrow(
                () ->
                    new RoutingConfigNotFoundException(
    ...
    for (RoutingRule rule : activeRulesInPriorityOrder(config)) {
      if (ConditionEvaluator.evaluate(rule.conditions(), workItem.fields())) {
        ...
        sample.stop(
            meterRegistry.timer(
                "routing.evaluation.duration", "workflowType", workItem.workflowType()));
    ...
    auditRepository.save(
        auditEntry(workItem, AuditEventType.ROUTING_FALLBACK, config.defaultGroupId()));
    sample.stop(
        meterRegistry.timer(
            "routing.evaluation.duration", "workflowType", workItem.workflowType()));
  ...
  private static AuditEntry auditEntry(
      WorkItemToRoute workItem, AuditEventType eventType, String assignedGroupId) {
