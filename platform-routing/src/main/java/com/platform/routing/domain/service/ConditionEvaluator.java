...
/**
 * Evaluates a ConditionNode tree against a WorkItem.fields map. Package-private — consumed only by
 * RoutingService and PriorityScoringEngine.
 *
 * <p>LEAF nodes: resolve the dot-notation field path via FieldPathResolver, then apply the
 * operator. GT/GTE/LT/LTE use BigDecimal comparison to support MONETARY and DECIMAL fields with
 * full precision.
 *
 * <p>GROUP nodes: combine children with AND (all must match) or OR (any must match).
 */
