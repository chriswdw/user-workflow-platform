...
/**
 * Unit test for the concurrent-duplicate edge case in IngestionService: two threads both pass
 * idempotencyRepository.exists() before either saves, then the second INSERT hits the DB unique
 * constraint and the adapter throws DuplicateIdempotencyKeyException. The service must handle this
 * gracefully.
 */
class IngestionServiceConcurrentDuplicateTest {
  ...
  void ingest_returnsDuplicate_whenWorkItemRepositoryThrowsDuplicateKey() {
    ...
    var throwingWorkItemRepo =
        (com.platform.ingestion.domain.ports.out.IIngestionWorkItemRepository)
            workItem -> {
              throw new DuplicateIdempotencyKeyException(workItem.idempotencyKey());
            };
    ...
    var service =
        new IngestionService(
            configRepo,
            idempotencyRepo,
            throwingWorkItemRepo,
            auditRepo,
            groupPort,
            event -> {},
            new SimpleMeterRegistry());

    RawInboundRecord inboundRecord =
        new RawInboundRecord(
            TENANT,
            WORKFLOW_TYPE,
            SourceType.KAFKA,
            "src-1",
            Map.of("tradeRef", "TRD-CONCURRENT"),
            "system");
