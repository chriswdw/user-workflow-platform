...
/**
 * Cucumber step definitions for the ingestion feature. Cucumber creates a new instance per scenario
...
public class IngestionStepDefinitions {
  ...
  private final InMemoryIngestionConfigRepository configRepo =
      new InMemoryIngestionConfigRepository();
  private final InMemoryIdempotencyKeyRepository idempotencyRepo =
      new InMemoryIdempotencyKeyRepository();
  private final InMemoryIngestionWorkItemRepository workItemRepo =
      new InMemoryIngestionWorkItemRepository();
  ...
  @Given(
      "an ingestion config for tenant {string} workflow {string} source {string} policy {string} with mappings:")
  public void ingestionConfig(
      String tenant, String workflow, String source, String policy, DataTable dataTable) {
    ...
    saveConfig(
        ...
        dataTable,
        UnknownColumnPolicy.valueOf(policy));
  }
  ...
  public void theWorkItemSourceTypeIs(String expectedSourceType) {
    var workItem = ((IngestionResult.Created) result).workItem();
    assertThat(workItem.source())
        .as("source type")
        .isEqualTo(SourceType.valueOf(expectedSourceType));
  }
