...
/**
 * Domain service implementing the work item ingestion use case. No framework dependencies —
 * constructor-injected output ports only.
 *
 * <p>Algorithm: 1. Load ingestion config 2. Map source fields → nested WorkItem.fields via
 * FieldMapping 3. Reject if any required source field is absent 4. Reject or ignore unknown source
 * fields per UnknownColumnPolicy 5. Compute idempotency key (EXPLICIT_FIELD or COMPOSITE_HASH) 6.
 * If duplicate: write DUPLICATE_INGESTION_DISCARDED audit, return Duplicate 7. Assign resolution
 * group via routing port 8. Build and save WorkItem; save idempotency key; write INGESTION audit
 */
public class IngestionService implements IIngestRecordUseCase {
  ...
  public IngestionResult ingest(RawInboundRecord inboundRecord) {
    IngestionConfig config =
        configRepository
            ...
            .orElseThrow(
                () ->
                    new IngestionConfigNotFoundException(
                        inboundRecord.tenantId(),
                        inboundRecord.workflowType(),
                        inboundRecord.sourceType()));

    // Map fields and validate
    Set<String> declaredSourceFields =
        config.fieldMappings().stream().map(FieldMapping::sourceField).collect(Collectors.toSet());
    ...
    for (FieldMapping mapping : config.fieldMappings()) {
      ...
      if (value == null) {
        if (mapping.required()) {
          ...
          return new IngestionResult.Rejected("Required field missing: " + mapping.sourceField());
        }
      } else {
    ...
    // Handle unknown columns — IGNORE and WARN are no-ops in the domain (WARN is
    // observability-only)
    if (config.unknownColumnPolicy() == UnknownColumnPolicy.REJECT) {
      Optional<String> firstUnknown =
          inboundRecord.rawFields().keySet().stream()
    ...
    // Duplicate check
    if (idempotencyRepository.exists(
        inboundRecord.tenantId(), inboundRecord.workflowType(), idempotencyKey)) {
      auditRepository.save(
          auditEntry(
              inboundRecord.tenantId(),
              inboundRecord.makerUserId(),
              AuditEventType.DUPLICATE_INGESTION_DISCARDED,
              idempotencyKey,
              null,
              null));
    ...
    IGroupAssignmentPort.AssignmentResult assignment =
        groupAssignmentPort.assignGroup(
            inboundRecord.tenantId(), inboundRecord.workflowType(), mappedFields);
    ...
    WorkItem workItem =
        new WorkItem(
            ...
            null,
            null,
            null, // priority fields — computed separately
            null,
            null, // pendingChecker fields
            1,
            inboundRecord.makerUserId(),
            Instant.now(),
            Instant.now());

    // Persist — catch concurrent duplicate: two threads may both pass exists() before either
    // inserts
  ...
  private static String computeIdempotencyKey(
      RawInboundRecord inboundRecord, IngestionConfig config) {
