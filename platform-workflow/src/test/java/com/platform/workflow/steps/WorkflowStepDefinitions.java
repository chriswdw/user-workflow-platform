...
/**
 * Cucumber step definitions for the workflow feature. Cucumber creates a new instance per scenario
 * — all fields are per-scenario state. Domain service wired with in-memory doubles; no Spring
 * context, no database.
 */
public class WorkflowStepDefinitions {
  ...
  private final InMemoryWorkflowConfigRepository configRepo =
  ...
  @Given(
      "a work item {string} of workflow type {string} in state {string} assigned to group {string}")
  ...
  public void theWorkItemIsInState(String state) {
    workItemRepo
        .findById(tenantId, workItemId)
        .ifPresent(item -> workItemRepo.save(item.withStatus(state)));
  }

  @Given(
      "the workflow config has a transition {string} from {string} to {string} for role {string} with REASSIGN_GROUP action targeting {string}")
  public void addTransitionWithReassign(
      String name, String from, String to, String role, String targetGroup) {
    pendingTransitions.add(
        new WorkflowTransition(
            name,
            from,
  ...
  @Given(
      "the workflow config has a transition {string} from {string} to {string} for role {string} requiring field {string} EXISTS")
  public void addTransitionWithExistsValidation(
      String name, String from, String to, String role, String field) {
    addTransitionWithValidation(name, from, to, role, new ValidationRule(field, "EXISTS", null));
  }

  @Given(
      "the workflow config has a transition {string} from {string} to {string} for role {string} requiring field {string} EQ {string}")
  public void addTransitionWithEqValidation(
      String name, String from, String to, String role, String field, String value) {
    addTransitionWithValidation(name, from, to, role, new ValidationRule(field, "EQ", value));
  }

  @Given(
      "the workflow config has a transition {string} from {string} to {string} for role {string} requiring field {string} NEQ {string}")
  public void addTransitionWithNeqValidation(
      String name, String from, String to, String role, String field, String value) {
    addTransitionWithValidation(name, from, to, role, new ValidationRule(field, "NEQ", value));
  }

  @Given(
      "the workflow config has a transition {string} from {string} to {string} for role {string} requiring field {string} with unsupported operator {string} and value {string}")
  ...
  private void addTransitionWithValidation(
      String name, String from, String to, String role, ValidationRule rule) {
    pendingTransitions.add(
        new WorkflowTransition(
            name,
            from,
  ...
  public void executeTransitionWithFields(
      String userId, String role, String transitionName, DataTable dataTable) {
  ...
  public void attemptsTransition(String userId, String role, String transitionName) {
    try {
      workflowService.transition(
          new TransitionCommand(workItemId, tenantId, transitionName, userId, role, Map.of()));
    } catch (ForbiddenTransitionException
  ...
  public void aStateTransitionAuditEntryIsWritten() {
    assertThat(auditRepo.all()).anyMatch(e -> e.eventType() == AuditEventType.STATE_TRANSITION);
  }

  @Then("a GROUP_REASSIGNMENT audit entry is written")
  public void aGroupReassignmentAuditEntryIsWritten() {
    assertThat(auditRepo.all()).anyMatch(e -> e.eventType() == AuditEventType.GROUP_REASSIGNMENT);
  }
  ...
  public void aFieldUpdateAuditEntryIsWritten() {
    assertThat(auditRepo.all()).anyMatch(e -> e.eventType() == AuditEventType.FIELD_UPDATE);
  }
  ...
  private void saveConfig() {
    ...
    configRepo.save(
        new WorkflowConfig(
            ...
            "UNDER_REVIEW",
            states,
            List.copyOf(pendingTransitions),
            true));
  }
