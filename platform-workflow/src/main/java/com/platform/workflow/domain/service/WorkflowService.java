...
/**
 * Domain service implementing the workflow state machine. No framework dependencies —
 * constructor-injected output ports only.
 *
 * <p>Algorithm: 1. Load work item and active workflow config 2. Find the named transition; validate
 * fromState matches current status 3. Check actor role is in transition.allowedRoles 4. Evaluate
 * validationRules against work item fields — fail fast on first violation 5. Execute REASSIGN_GROUP
 * actions (other action types deferred to adapter layer via events) 6. Apply state transition,
 * write audit entry, save work item
 */
public class WorkflowService implements ITransitionWorkItemUseCase {
  ...
  public WorkItem transition(TransitionCommand command) {
    ...
    WorkItem workItem =
        workItemRepository
            .findById(command.tenantId(), command.workItemId())
    ...
    WorkflowConfig config =
        workflowConfigRepository
            ...
            .orElseThrow(
                () ->
                    new WorkflowConfigNotFoundException(
                        workItem.workflowType(), workItem.tenantId()));

    WorkflowTransition transition =
        config
            .findTransition(command.transitionName())
            .orElseThrow(
                () ->
    ...
    if (!transition.allowedRoles().contains(command.actorRole())) {
      throw new ForbiddenTransitionException(
          "Role '"
              + command.actorRole()
              + "' is not permitted to execute transition '"
              + command.transitionName()
              + "'");
    }
    ...
    if (!command.additionalFields().isEmpty()) {
      ...
      List<AuditEntry.ChangedField> fieldChanges =
          command.additionalFields().entrySet().stream()
              .map(
                  e ->
                      new AuditEntry.ChangedField(
                          e.getKey(),
    ...
    for (TransitionAction action : transition.actions()) {
      if (action.type() == com.platform.workflow.domain.model.TransitionActionType.REASSIGN_GROUP) {
        ...
        auditRepository.save(
            groupReassignmentAuditEntry(command, workItem, previousGroup, targetGroup));
      }
    }

    // Apply state transition
    String previousState = updated.status();
    updated = updated.withStatus(transition.toState()).withMakerUserId(command.actorUserId());

    auditRepository.save(
        stateTransitionAuditEntry(command, updated, previousState, transition.toState()));

    WorkItem saved = workItemRepository.save(updated);
    eventPublisher.publish(
        stateTransitionEvent(command, saved, previousState, transition.toState()));
    sample.stop(
        meterRegistry.timer(
  ...
  private static boolean passes(ValidationRule rule, WorkItem workItem) {
    return switch (rule.operator()) {
      ...
      case "EQ" ->
          FieldPathResolver.resolve(workItem.fields(), rule.field())
      ...
      case "NEQ" ->
          FieldPathResolver.resolve(workItem.fields(), rule.field())
              ...
              .orElse(false);
      default ->
          throw new IllegalArgumentException("Unsupported validation operator: " + rule.operator());
    };
  }

  private static DomainEvent stateTransitionEvent(
      TransitionCommand command, WorkItem workItem, String previousState, String newState) {
  ...
  private static AuditEntry stateTransitionAuditEntry(
      TransitionCommand command, WorkItem workItem, String previousState, String newState) {
  ...
  private static AuditEntry groupReassignmentAuditEntry(
      TransitionCommand command, WorkItem workItem, String previousGroup, String newGroup) {
    return new AuditEntry(
        UUID.randomUUID().toString(),
        workItem.tenantId(),
        workItem.id(),
  ...
  private static AuditEntry fieldUpdateAuditEntry(
      TransitionCommand command, WorkItem workItem, List<AuditEntry.ChangedField> changedFields) {
    return new AuditEntry(
        UUID.randomUUID().toString(),
        workItem.tenantId(),
        workItem.id(),
  ...
  private static void setNestedValue(
      java.util.Map<String, Object> map, String dotPath, Object value) {
